...
local function getItemHeight(item)
    ...
    if ProjectState.skeetMode then
        ...
        elseif t == "button" then return max(28, #(item.buttons or {}) * 28)
...
local function drawWidget(item, rowX, rowY, rowW, trans, click, rightClick, popupBlocking)
    ...
    elseif t == "button" then
        ...
        local nB = #btns
        local gap = ProjectState.skeetMode and 6 or 10
        ...
        for bi, b in ipairs(btns) do
            ...
            local hf = b._hf
            rect(bx, by, bw, bh, lerpColor(Theme.surface2, Theme.surface3, hf), 30, 0, trans)
            rect(bx + 1, by + 1, bw - 2, floor(bh / 2), WHITE, 31, 0, 0.025 * trans)
            strokeRect(bx, by, bw, bh, lerpColor(Theme.border, ProjectState._accentMid, hf), 32, 0, trans)
    ...
    elseif t == "slider" then
        ...
        local barH = ProjectState.skeetMode and 5 or 8
        local barR = ProjectState.skeetMode and 0 or 4
        ...
        if not ProjectState.skeetMode then circ(knobX, syBar + 4, item.animatedRadius, WHITE, 32, true, 1, 24, trans) end
...
local function drawSectionCard(section, colX, sy, colW, secH, clipTop, clipBottom, click, held, rightClick, popupBlocking)
    ...
    if headerH > 0 then
        ...
        if ha > 0.01 then
            ...
            if section._nameU ~= section.name then
                ...
                section._nameUpper = ProjectState.skeetMode and section.name or string.upper(section.name)
            end
    ...
    rect(colX, drawY, colW, drawH, Theme.surface, 28, 0, AL.card * cf)
    strokeRect(colX, drawY, colW, drawH, Theme.border, 33, 0, AL.cardStrk * cf)
    local rowW = colW - 24
    local rowX = colX + 12
    ...
    for i, item in ipairs(section.items) do
        ...
        if rowY + ih < clipTop - 2 or rowY > cardBottom + 2 then
            ...
            strokeRect(rowX - 3, rowY - 3, rowW + 6, ih + 6, ProjectState._accentMid, 33, 0, fa * trans)
...
function ProjectState.drawSkeetWindow(click, held, rightClick)
    ...
    ProjectState._winRect = { x = x, y = y, w = w, h = h, th = 0, v = v }
