...
-- Audience: experienced farm operator · Use: fast runtime control · Tone: technical, austere
...
local ProjectState = {
    ...
    smartFps = false, checkboxStyle = false, railOnly = false, skeetMode = false, compactSkeet = false, neverloseMode = false,
    errorCount = 0,
}
local menuKey = "p"
local keybindItems = {}
...
local function hideWindowDrawings()
    if ProjectState.windowImagesHidden then return end
    local ok = pcall(function()
        local function hideImage(img, cache)
            if img and (not cache or cache[7] ~= false) then
                img.Visible = false
                if cache then cache[7] = false end
            end
        end
        for _, t in ipairs(ProjectState.tabs) do
            hideImage(t._img, t._ic)
            hideImage(t._imgA, t._icA)
            if t.subs then for _, sb in ipairs(t.subs) do
                hideImage(sb._img, sb._ic)
                hideImage(sb._imgA, sb._icA)
            end end
            for _, sec in ipairs(t.sections) do for _, it in ipairs(sec.items) do
                hideImage(it._img, it._imgIc)
                hideImage(it._ddImg, it._ddIc)
            end end
        end
        hideImage(ProjectState._gearImg, ProjectState._gearIc)
        hideImage(ProjectState.bgImg, ProjectState._bgIc)
        hideImage(ProjectState.avatarImg, ProjectState._avatarIc)
        hideImage(ProjectState.logoImg, ProjectState._logoIc)
        hideImage(ProjectState.iconImg, ProjectState._iconIc)
    end)
    if ok then ProjectState.windowImagesHidden = true end
end
...
function ProjectState.spectrumLine(x, y, w, z, alpha)
    local colors = {
        c3(69, 170, 242), c3(109, 145, 221), c3(151, 118, 196),
        c3(202, 88, 151), c3(252, 92, 101), c3(253, 126, 83),
        c3(253, 164, 68), c3(254, 211, 48),
    }
    local segments = #colors
    local used = 0
    for i = 1, segments do
        local nextUsed = floor(w * i / segments + 0.5)
        rect(x + used, y, max(1, nextUsed - used), 2, colors[i], z, 0, alpha)
        used = nextUsed
    end
end
...
ProjectState.placeImg = function(img, c, px, py, sx, sy, z, tr, vis, rounding)
    if not img or not c then return end
    if c._img ~= img then
        c._img = img
        for i = 1, 8 do c[i] = nil end
    end
    pcall(function()
    ...
    if rounding ~= nil and c[8] ~= rounding then
        local ok = pcall(function() img.Rounding = rounding end)
        if ok then c[8] = rounding end
    end
end
...
function ui:_dbgOpenFeatureSettings(h)
    ...
    if item and item.settings then
        ProjectState.featureSettings = {
            item = item,
            x = ProjectState.x + ProjectState.w - 268,
            y = ProjectState.y + 58,
            anim = 0
        }
    end
...
function dDropdown(x, y, w, item, anchorX, anchorY, anchorW, anchorH)
    ...
    ProjectState.dropdown = { item = item, choices = copyArray(item.choices), value = item.value,
        ...
        anchorX = anchorX or x, anchorY = anchorY or (y - 26), anchorW = anchorW or w, anchorH = anchorH or 24,
...
local function drawDropdown(click, rightClick)
    ...
    local anchorHovered = over(dd.anchorX or x, dd.anchorY or (y - 26), dd.anchorW or w, dd.anchorH or 24)
    if click and anchorHovered and not dd.closing then
        dd.closing = true
...
ProjectState.visibleFeatureOptions = function(popup, source)
    local visible = popup.visibleOptions
    if not visible then
        visible = {}
        popup.visibleOptions = visible
    else
        for index = #visible, 1, -1 do visible[index] = nil end
    end

    for _, option in ipairs(source) do
        local shown = option.hidden ~= true
        if shown and type(option.visibleWhen) == "function" then
            local ok, result = pcall(option.visibleWhen)
            shown = ok and result == true
        end
        if shown then visible[#visible + 1] = option end
    end

    if popup.openOption then
        local stillVisible = false
        for _, option in ipairs(visible) do
            if option == popup.openOption then
                stillVisible = true
                break
            end
        end
        if not stillVisible then popup.openOption = nil end
    end
    return visible
end

function ProjectState.drawNeverloseFeatureSettings(click, held)
    local popup = ProjectState.featureSettings
    local settings = popup and popup.item and popup.item.settings
    if not settings then
        ProjectState.featureSettings = nil
        return click
    end

    popup.anim = approach(popup.anim or 0, 1, 22)
    local a = popup.anim
    local options = ProjectState.visibleFeatureOptions(popup, settings.options or {})
    local rowH, w = 46, 252
    local expandedChoices = popup.openOption and (popup.openOption.choices or {}) or {}
    local expandedH = popup.openOption and (#expandedChoices * 23 + 5) or 0
    local h = 47 + #options * rowH + expandedH + 9
    local vw, vh = viewportSize()
    local x = clamp(popup.x, 8, max(8, vw - w - 8))
    local y = clamp(popup.y, 8, max(8, vh - h - 8)) + (1 - a) * -7
    local accent = ProjectState._accentMid

    rect(x - 6, y + 6, w + 12, h + 7, c3(0, 0, 0), 248, 13, 0.22 * a)
    rect(x - 2, y + 2, w + 4, h + 3, c3(3, 6, 13), 249, 11, 0.5 * a)
    rect(x, y, w, h, Theme.surface2, 250, 9, 0.995 * a)
    strokeRect(x, y, w, h, Theme.border, 251, 9, 0.88 * a)

    rect(x + 1, y + 1, w - 2, 39, Theme.sidebar, 251, 8, 0.98 * a)
    rect(x + 1, y + 32, w - 2, 8, Theme.sidebar, 251, 0, 0.98 * a)
    lineD(x + 1, y + 40, x + w - 1, y + 40, Theme.border, 252, 1, 0.7 * a)
    circ(x + 15, y + 20, 3, accent, 253, true, 1, 14, a)
    txt(settings.title or "Options", x + 26, y + 12, WHITE, 13, FontBold, 253, false, false, w - 64, AL.text * a)

    local closeHov = over(x + w - 31, y + 8, 23, 23)
    rect(x + w - 31, y + 8, 23, 23, accent, 252, 6, (closeHov and 0.13 or 0) * a)
    lineD(x + w - 24, y + 15, x + w - 15, y + 24, closeHov and accent or Theme.sub, 253, 1.3, a)
    lineD(x + w - 15, y + 15, x + w - 24, y + 24, closeHov and accent or Theme.sub, 253, 1.3, a)
    ...
    for index, option in ipairs(options) do
        local oy = y + 44 + (index - 1) * rowH + optionOffset
        local kind = option.type or "toggle"
        txt(option.label or "Option", x + 14, oy + 7, WHITE, 12, FontSystem, 253, false, false, w - 28, AL.label * a)

        if kind == "toggle" then
            local tw, th = 34, 18
            local tx, ty = x + w - tw - 14, oy + 5
            local hov = over(tx - 3, ty - 3, tw + 6, th + 6)
            option._anim = approach(option._anim or (option.value and 1 or 0), option.value and 1 or 0, 18)
            local f = option._anim
            rect(tx, ty, tw, th, lerpColor(Theme.trackOff, accent, f), 253, 9, a)
            strokeRect(tx, ty, tw, th, lerpColor(Theme.border, accent, f), 254, 9, (0.65 + 0.25 * (hov and 1 or 0)) * a)
            circ(tx + 9 + 16 * f, ty + 9, 6, WHITE, 255, true, 1, 18, (0.84 + 0.16 * f) * a)
        ...
        elseif kind == "slider" then
            ...
            local sx, sy, sw = x + 14, oy + 31, w - 28
            local alpha = maxV == minV and 0 or (value - minV) / (maxV - minV)
            rect(sx, sy, sw, 4, Theme.sliderTrack, 253, 2, a)
            rect(sx, sy, sw * alpha, 4, accent, 254, 2, a)
            circ(sx + sw * alpha, sy + 2, 4, WHITE, 255, true, 1, 18, a)
            local valueText = tostring(option.value) .. tostring(option.suffix or "")
            local valueW = textWidth(valueText, 11, FontSystem) + 12
            rect(x + w - valueW - 14, oy + 5, valueW, 18, Theme.surface3, 253, 5, a)
            txt(valueText, x + w - valueW - 8, textTop(oy + 5, 18, 11), WHITE, 11, FontSystem, 254, false, false, valueW - 8, AL.text * a)
            local hov = over(sx, sy - 7, sw, 18)
        ...
        elseif kind == "dropdown" then
            local fx, fy, fw, fh = x + 14, oy + 24, w - 28, 20
            local fieldHov = over(fx, fy, fw, fh)
            local isOpen = popup.openOption == option
            rect(fx, fy, fw, fh, Theme.surface3, 253, 5, a)
            strokeRect(fx, fy, fw, fh, isOpen and accent or Theme.border, 254, 5, (isOpen and 0.9 or (fieldHov and 0.8 or 0.6)) * a)
            txt(tostring(option.value or "-"), fx + 8, textTop(fy, fh, 11), WHITE, 11, FontSystem, 255, false, false, fw - 28, AL.text * a)
            lineD(fx + fw - 15, fy + 8, fx + fw - 11, fy + 12, Theme.sub, 255, 1, a)
            lineD(fx + fw - 11, fy + 12, fx + fw - 7, fy + 8, Theme.sub, 255, 1, a)
            ...
            if isOpen then
                ...
                for choiceIndex, choice in ipairs(choices) do
                    local cy = listY + (choiceIndex - 1) * 23
                    local choiceHov = over(fx, cy, fw, 21)
                    local selected = option.value == choice
                    rect(fx, cy, fw, 21, selected and accent or Theme.surface3, 253, 5, (selected and 0.18 or (choiceHov and 0.98 or 0.86)) * a)
                    strokeRect(fx, cy, fw, 21, selected and accent or Theme.border, 254, 5, (selected and 0.75 or 0.5) * a)
                    txt(tostring(choice), fx + 8, textTop(cy, 21, 11), selected and WHITE or Theme.sub, 11, FontSystem, 255, false, false, fw - 16, a)
                ...
                optionOffset = optionOffset + #choices * 23 + 5
            end
        end

        if index < #options then
            lineD(x + 14, oy + rowH - 1, x + w - 14, oy + rowH - 1, Theme.border, 252, 1, 0.35 * a)
        end
    end
    ...
    if click and not over(x - 5, y - 5, w + 10, h + 10) then
        ProjectState.featureSettings = nil
        return false
    end
    return click
...
function ProjectState.drawFeatureSettings(click, held)
    ...
    if ProjectState.neverloseMode then
        return ProjectState.drawNeverloseFeatureSettings(click, held)
    end
...
function ProjectState.resolveDynamicValue(owner, value, now)
    if type(value) ~= "function" then return value end
    if not owner._dynamicAt or now >= owner._dynamicAt then
        local ok, result = pcall(value)
        owner._dynamicValue = ok and result or nil
        owner._dynamicAt = now + 0.1
    end
    return owner._dynamicValue
end
...
local function drawBoxes(click, held)
    ...
    for _, box in ipairs(boxes) do
        if box.visible ~= false and box.alive ~= false then
            ...
            local accentMid = ProjectState._accentMid
            local lines = box._resolvedLines
            if not lines then
                lines = {}
                box._resolvedLines = lines
            end
            local lineCount = 0
            ...
            for _, ln in ipairs(box.lines) do
                ...
                if kind == "stat" then
                    local v = ProjectState.resolveDynamicValue(ln, ln.value, now)
                    ...
                    if v ~= "" then
                        ...
                        local resolved = lines[lineCount] or {}
                        lines[lineCount] = resolved
                        resolved.kind = "stat"
                        resolved.label = lbl
                        resolved.text = val
                        resolved.pct = nil
                        resolved.color = ln.color
                ...
                elseif kind == "bar" then
                    ...
                    if v ~= nil then
                        ...
                        resolved.label = nil
                        resolved.text = nil
                        resolved.pct = clamp(pct, 0, 100)
                        resolved.color = ln.color
                        maxW = max(maxW, 170)
                    end
                else
                    local text = ProjectState.resolveDynamicValue(ln, ln.value, now)
                    ...
                    resolved.pct = nil
                    resolved.color = ln.color
                    maxW = max(maxW, textWidth(text, 12, FontSystem) + 24)
                end
            end
            for index = #lines, lineCount + 1, -1 do
                lines[index] = nil
            end
...
local function drawMinBubble(click, held)
    ...
    if et > 0.4 then
        ...
        if ProjectState.iconImg then
            local ls = rw - 8
            ProjectState._iconIc = ProjectState._iconIc or {}
            ProjectState.placeImg(
                ProjectState.iconImg,
                ProjectState._iconIc,
                cx - ls / 2,
                cy - ls / 2,
                ls,
                ls,
...
local function drawWidget(item, rowX, rowY, rowW, trans, click, rightClick, popupBlocking)
    ...
    if t == "label" then
        if item.labelFn then
            local value = ProjectState.resolveDynamicValue(
                item,
                item.labelFn,
                clock() or 0
            )
            if value ~= nil then item.label = tostring(value) end
        end
    ...
    elseif t == "checkbox" then
        ...
        if ProjectState.checkboxStyle then
            local bs = 11
            px = rowX
            local py = rowY + (ProjectState.skeetMode and 5 or 10)
            ...
            checkLabelX = rowX + bs + 7
        else
            local pw, ph = 38, 20
            px = rowX + rowW - pw
            local py = rowY + (ProjectState.neverloseMode and 2 or 6)
        ...
        if not ProjectState.checkboxStyle then rightX = px - 8 end
    ...
    elseif t == "slider" then
        ...
        local barH = ProjectState.skeetMode and 5 or (ProjectState.neverloseMode and 4 or 8)
        local barR = ProjectState.skeetMode and 0 or (ProjectState.neverloseMode and 2 or 4)
...
local function secHeaderH(s)
    ...
    return HEADER_H + ((s.desc and s.desc ~= "") and 16 or 0)
end
local function drawSectionCard(section, colX, sy, colW, secH, clipTop, clipBottom, click, held, rightClick, popupBlocking)
    ...
    local groupTop = ProjectState.neverloseMode and sy or (sy + 7)
    if headerH > 0 then
        ...
        if ha > 0.01 then
            ...
            if section.desc then txt(section.desc, colX + 13, sy + 17, Theme.sub, 10, FontSystem, 35, false, false, colW - 26, AL.dim * ha) end
    ...
    if (section._cA or 0) > 0.98 then
        for _, it in ipairs(section.items) do
            if it._img then
                pcall(function() it._img.Visible = false end)
                if it._imgIc then it._imgIc[7] = false end
            end
            if it._ddImg then
                pcall(function() it._ddImg.Visible = false end)
                if it._ddIc then it._ddIc[7] = false end
            end
        end
        return click, rightClick
    end
    local cardTop = groupTop
    ...
    local gap = ProjectState.skeetMode and 5 or (ProjectState.neverloseMode and 6 or (ProjectState.rowLines and 8 or 10))
    local cardBottom = min(clipBottom, sy + secH)
    for i, item in ipairs(section.items) do
        local ih = getItemHeight(item)
        if rowY + ih < clipTop - 2 or rowY > cardBottom + 2 then
            if item._img then
                pcall(function() item._img.Visible = false end)
                if item._imgIc then item._imgIc[7] = false end
            end
            ...
            strokeRect(rowX - 3, rowY - 3, rowW + 6, ih + 6, ProjectState._accentMid, 33, ProjectState.neverloseMode and 5 or 0, fa * trans)
...
local function drawSections(tab, click, held, rightClick, px, contY, pw, contH)
    ...
    for si, s in ipairs(tab.sections) do
        ...
        elseif s.side == "Right" then
            tx = px + colW + 8; ty = rightY; tw = colW; rightY = rightY + s._h + 8
        else
            tx = px; ty = leftY; tw = colW; leftY = leftY + s._h + 8
        end
...
local FX_COUNT = { Snow = 20, Rain = 32 }
...
function ProjectState.drawSkeetWindow(click, held, rightClick)
    ...
    if held and ProjectState.drag then
        ...
        ProjectState.y = ProjectState.mouseY - ProjectState.drag.oy
        clampWindow()
    end
    ...
    strokeRect(x + 1, y + 1, w - 2, h - 2, c3(61, 65, 76), 11, 0, v)
    strokeRect(x + 3, y + 3, w - 6, h - 6, c3(61, 65, 76), 12, 0, v)
    strokeRect(x + 5, y + 5, w - 10, h - 10, c3(8, 8, 8), 13, 0, v)
    ProjectState.spectrumLine(x + inset, y + inset, w - inset * 2, 14, v)

    local railX, railY = x + inset, y + inset + 3
    local railH = h - inset * 2 - 3
    ...
    for i, tab in ipairs(ProjectState.tabs) do
        if not tab.hidden then
            ...
            local hov = noPopup and over(railX, cy, railW, cellH)
            rect(railX, cy, railW, cellH, active and c3(17, 17, 17) or c3(12, 12, 12), 16, 0, v)
            if active then
                lineD(railX, cy, railX + railW, cy, c3(50, 48, 52), 17, 1, v)
                lineD(railX, cy + cellH, railX + railW, cy + cellH, c3(50, 48, 52), 17, 1, v)
            else
    ...
    if ProjectState.activeTab then
        click, rightClick = drawSections(ProjectState.activeSub or ProjectState.activeTab, click, held, rightClick, contentX, contentY, contentW, contentH)
    end

    local dragHov = noPopup and over(railX + railW, railY, w - railW - inset * 2, 18)
    if click and dragHov and not ProjectState.drag and not ProjectState.resizeEdge then
        ProjectState.drag = { ox = ProjectState.mouseX - x, oy = ProjectState.mouseY - y }
        click = false
    end

    local resizeHov = over(x + w - 18, y + h - 18, 20, 20)
    if click and noPopup and resizeHov and not ProjectState.drag then
        ProjectState.resizeEdge = "br"
        ProjectState.resizeStart = { w = w, h = h, mx = ProjectState.mouseX, my = ProjectState.mouseY }
        click = false
    end
    lineD(x + w - 15, y + h - 6, x + w - 6, y + h - 15, c3(75, 75, 78), 18, 1, 0.75 * v)
    lineD(x + w - 11, y + h - 6, x + w - 6, y + h - 11, c3(75, 75, 78), 18, 1, 0.75 * v)

    local winRect = ProjectState._winRect
    if not winRect then winRect = {}; ProjectState._winRect = winRect end
    winRect.x, winRect.y, winRect.w = x, y, w
    winRect.h, winRect.th, winRect.v = h, 0, v
...
function ProjectState.drawNeverloseWindow(click, held, rightClick)
    ...
    local noPopup = not ProjectState.dropdown
        ...
        and not ProjectState.keyMenu
        and not ProjectState.spotlightOpen
    ...
    if ProjectState.drag and ProjectState.drag.tx then
        ...
        clampWindow()
        local dx, dy = ProjectState.x - oldX, ProjectState.y - oldY
    ...
    if held and ProjectState.resizeEdge then
        local rs = ProjectState.resizeStart
        ProjectState.wTarget = max(560, rs.w + (ProjectState.mouseX - rs.mx))
        ProjectState.hTarget = max(380, rs.h + (ProjectState.mouseY - rs.my))
    end
    ...
    if click and noPopup and over(x, y, w, headerH) then
        ProjectState.drag = {
            ox = ProjectState.mouseX - x,
            oy = ProjectState.mouseY - y
        }
        click = false
    end
    ...
    if click and resizeHov and not ProjectState.drag then
        ProjectState.resizeEdge = "br"
        ProjectState.resizeStart = {
            w = w,
            h = h,
            mx = ProjectState.mouseX,
            my = ProjectState.mouseY
        }
        click = false
    end
    ...
    if ProjectState.contentFade > 0.997 then ProjectState.contentFade = 1 end
    if ProjectState.activeTab then
        click, rightClick = drawSections(
            ProjectState.activeSub or ProjectState.activeTab,
            click,
            held,
            rightClick,
            contentX,
    ...
    if not winRect then winRect = {}; ProjectState._winRect = winRect end
    winRect.x, winRect.y, winRect.w = x, y, w
    winRect.h, winRect.th, winRect.v = h, headerH, v
    return click, held, rightClick
end

local function drawWindow(click, held, rightClick)
    ProjectState.windowImagesHidden = false
    if ProjectState.skeetMode then
        return ProjectState.drawSkeetWindow(click, held, rightClick)
    end
    if ProjectState.neverloseMode then
        return ProjectState.drawNeverloseWindow(click, held, rightClick)
    end
    ...
    if activeView ~= ProjectState._tabSeen then
        ...
        pcall(function()
            for _, t in ipairs(ProjectState.tabs) do
                ...
                if t.subs then for _, sb in ipairs(t.subs) do for _, sec in ipairs(sb.sections) do for _, it in ipairs(sec.items) do
                    if it._img then
                        it._img.Visible = false
                        if it._imgIc then it._imgIc[7] = false end
                    end
    ...
    if ProjectState.railOnly then expand = 0 end
    local sw = sideMode and (swC + (swE - swC) * expand) or 0

    rect(x - 3, y + 3, w + 6, h + 6, c3(0, 0, 0), 9, 0, 0.72 * v)

    local baseAlpha = ProjectState.menuOpacity or 0.98
    rect(x, y, w, h, Theme.bg, 10, 0, baseAlpha * v)
    if ProjectState.bgImg then
        ...
        ProjectState._bgIc = ProjectState._bgIc or {}
        placeImg(
            ProjectState.bgImg,
            ProjectState._bgIc,
            x + (w - bw) / 2,
            ...
            bh,
            119999,
            (ProjectState.bgImgAlpha or 0.12) * v,
            v > 0.01
        )
    end
    local winRect = ProjectState._winRect
    if not winRect then winRect = {}; ProjectState._winRect = winRect end
    winRect.x, winRect.y, winRect.w = x, y, w
    winRect.h, winRect.th, winRect.v = h, topH, v
    strokeRect(x, y, w, h, c3(0, 0, 0), 12, 0, v)
    strokeRect(x + 1, y + 1, w - 2, h - 2, Theme.border, 13, 0, v)
    ProjectState.spectrumLine(x + 2, y + 2, w - 4, 14, v)

    if sideMode then
        rect(x, y + 4, sw, h - 5, Theme.sidebar, 11, 0, baseAlpha * v)
        rect(x + sw, y + topH, w - sw, h - topH, Theme.surface2, 11, 0, 0.38 * v)
        lineD(x + sw, y + 4, x + sw, y + h - 2, Theme.border, 12, 1, v)
        if ProjectState.activeTab then
            txt(string.upper((ProjectState.activeSub or ProjectState.activeTab).name), x + sw + 12, textTop(y + 3, topH - 3, 12), WHITE, 12, FontBold, 15, false, false, max(2, (w - sw) - 112), AL.text * v)
        end
    else
    ...
    do
        ...
        rect(b1, y + 4, b2 - b1, topH - 5, WHITE, 12, 0, 0.05 * sa)
        local s1, s2 = max(x + 2, sweep - 18), min(x + w - 2, sweep)
        rect(s1, y + 4, s2 - s1, topH - 5, ProjectState._accentMid, 12, 0, 0.04 * sa)
    end
    ...
    elseif ProjectState.logoImg and not sideMode then
        local lsz = bsz + 6
        ProjectState._logoIc = ProjectState._logoIc or {}
        placeImg(
            ProjectState.logoImg,
            ProjectState._logoIc,
            gemX - lsz / 2,
            gemY - lsz / 2,
            lsz,
            lsz,
    ...
    local function ctrlBtn(cxp, kind)
        ...
        rect(bx, by, bs, bs, accentMidT, 13, 0, 0.10 * hf * v)
        strokeRect(bx, by, bs, bs, accentMidT, 14, 0, 0.45 * hf * v)
    ...
    if ProjectState.logoImg then
        ...
        if ProjectState.iconImg then
            ProjectState._iconIc = ProjectState._iconIc or {}
            if ProjectState._iconIc[7] ~= false then
                pcall(function() ProjectState.iconImg.Visible = false end)
                ProjectState._iconIc[7] = false
            end
        end
        hdrX = hdrX + lsz + 9
    ...
    local pillH, subH, navR, subR = 28, 24, 0, 0
    ...
    for i, tab in ipairs(ProjectState.tabs) do
        ...
        rect(navPx, ty, navPw, pillH, navAcc, 61, navR, (0.22 * af + 0.09 * hf * (1 - af)) * v)
        ...
        if tab.icon then
            ...
            if IconBytes[tab.icon] and accData and not ProjectState.rainbow then
        ...
        if ProjectState.railOnly and inPill then tooltipReq(tab.name, ProjectState.mouseX, ProjectState.mouseY) end
        ...
        if nSubs > 0 then
            ...
            for j, sub in ipairs(tab.subs) do
                ...
                if sub.icon then
                    ...
                    if IconBytes[sub.icon] and sAcc and not ProjectState.rainbow then
    ...
    if not ProjectState.railOnly then
        ...
        if ProjectState.avatarImg then
            ProjectState._avatarIc = ProjectState._avatarIc or {}
            placeImg(
                ProjectState.avatarImg,
                ProjectState._avatarIc,
                acx - 14,
                acy - 14,
                28,
                28,
        ...
        if ProjectState.settingsTab then
            ...
            if IconBytes[gi] then
                ...
                if ProjectState._gearImg then
                    ProjectState._gearIc = ProjectState._gearIc or {}
                    placeImg(
                        ProjectState._gearImg,
                        ProjectState._gearIc,
                        gcx - isz / 2,
                        gcy - isz / 2,
                        isz,
                        isz,
        ...
        for i, tab in ipairs(ProjectState.tabs) do
            if not tab.hidden then
                ...
                if tab.icon then
                    ...
                    if IconBytes[tab.icon] and tAcc and not ProjectState.rainbow then
                        if not tab._img then tab._ic = {}; pcall(function() tab._img = Drawing.new("Image"); tab._img.Data = IconBytes[tab.icon] end) end
                    ...
                    elseif ProjectState.rainbow and Icons[tab.icon] then
                        if tab._img then placeImg(tab._img, tab._ic, tx + 10, iiy, 16, 16, 169998, 0, false) end
                        if tab._imgA then placeImg(tab._imgA, tab._icA, tx + 10, iiy, 16, 16, 169999, 0, false) end
                        drawIcon(tab.icon, tx + 10, iiy, 16, accentMidT, 15, la)
                    elseif IconBytes[tab.icon] then
                        if not tab._img then tab._ic = {}; pcall(function() tab._img = Drawing.new("Image"); tab._img.Data = IconBytes[tab.icon] end) end
...
local function drawNotifications()
    ...
    if ProjectState.skeetMode then
        while #notes > 6 do remove(notes, 1) end
        local vw, vh = viewportSize()
        local baseY = floor(vh * 0.58)
        local i = 1
        while i <= #notes do
            local n = notes[i]
            ...
            if n.elapsed >= n.duration then
                ...
                local message = tostring(n.title or "")
                if n.description and n.description ~= "" then message = message .. "  " .. tostring(n.description) end
                local width = clamp(textWidth(message, 11, FontSystem) + 42, 220, 380)
                local fade = 1
                if n.elapsed < 0.18 then fade = n.elapsed / 0.18
                elseif n.duration - n.elapsed < 0.25 then fade = (n.duration - n.elapsed) / 0.25 end
                fade = clamp(fade, 0, 1)
                local nx = floor(vw / 2 - width / 2 + (1 - fade) * 18)
                local ny = baseY + (i - 1) * 24
                rect(nx + 2, ny + 2, width, 19, c3(0, 0, 0), 299, 0, 0.48 * fade)
                rect(nx, ny, width, 19, c3(9, 9, 10), 300, 0, 0.91 * fade)
                strokeRect(nx, ny, width, 19, c3(48, 50, 56), 301, 0, 0.82 * fade)
                drawIcon("skull", nx + 5, ny + 3, 13, c3(205, 205, 208), 302, 0.88 * fade)
                lineD(nx + 23, ny + 3, nx + 23, ny + 16, c3(54, 56, 62), 302, 1, 0.7 * fade)
                txt(message, nx + 30, textTop(ny, 19, 11), WHITE, 11, FontSystem, 302, false, false, width - 36, 0.9 * fade)
                local remain = clamp(1 - n.elapsed / n.duration, 0, 1)
                rect(nx, ny + 18, max(1, width * remain), 1, ProjectState._accentMid, 303, 0, 0.55 * fade)
                i = i + 1
            end
        end
        return
...
local function applyConfigData(data)
    ...
    local lock = ProjectState.lockedChrome
    if lock then
        Theme.bg, Theme.sidebar = lock.bg, lock.sidebar
        Theme.text, Theme.sub = lock.text, lock.sub
        Theme.surface, Theme.surface2 = lock.surface, lock.surface2
        Theme.surface3, Theme.border = lock.surface3, lock.border
        Theme.trackOff, Theme.sliderTrack = lock.trackOff, lock.sliderTrack
        Theme.accentA, Theme.accentB = lock.accentA, lock.accentB
        ProjectState.baseAccentA, ProjectState.baseAccentB = lock.accentA, lock.accentB
        ProjectState.rainbow = false
        ProjectState.menuOpacity = lock.opacity
        ProjectState.roundScale = lock.rounding
        ...
        ProjectState.neverloseMode = lock.neverloseMode == true
        ProjectState.searchStyle = lock.searchStyle
        ProjectState.tabLayout = lock.layout
        ui:SetMenuKey(lock.menuKey)
        ProjectState.uiFontName = lock.fontName
        ProjectState.uiFont = lock.font
        ProjectState.w, ProjectState.h = lock.w, lock.h
        ProjectState.wTarget, ProjectState.hTarget = lock.w, lock.h
        AL.card, AL.cardStrk, AL.hairline = lock.card, lock.cardStrk, lock.hairline
        ui:SetBackgroundEffect(nil)
...
function ProjectState.imgBytes(target)
...
function ui:SetBackgroundImage(url, alpha, wFrac, hFrac)
    ...
    task.spawn(function()
        pcall(function()
            local b = ProjectState.imgBytes(target)
            if b and #b > 12 then
...
local function finalDestroy()
    ...
    local function removeImage(owner, imageKey, cacheKey)
        ...
        if image then pcall(function() image:Remove() end) end
        if owner then
            owner[imageKey] = nil
            if cacheKey then owner[cacheKey] = nil end
        end
    end
    local function removeOwnerImages(owner)
        removeImage(owner, "_img", "_ic")
        removeImage(owner, "_imgA", "_icA")
        removeImage(owner, "_ddImg", "_ddIc")
    end
    local function removeSectionImages(sections)
        for _, section in ipairs(sections or {}) do
            for _, item in ipairs(section.items or {}) do
                removeOwnerImages(item)
            end
        end
    end
    for _, tab in ipairs(ProjectState.tabs) do
        removeOwnerImages(tab)
        removeSectionImages(tab.sections)
        for _, sub in ipairs(tab.subs or {}) do
            removeOwnerImages(sub)
            removeSectionImages(sub.sections)
        end
    end
    removeImage(ProjectState, "_gearImg", "_gearIc")
    removeImage(ProjectState, "bgImg", "_bgIc")
    removeImage(ProjectState, "avatarImg", "_avatarIc")
    removeImage(ProjectState, "logoImg", "_logoIc")
    removeImage(ProjectState, "iconImg", "_iconIc")
    ProjectState.bgImgUrl = nil
    ProjectState.logoSrc = nil
    ProjectState.iconSrc = nil
    ProjectState.boxes = {}
...
local function step()
    ...
    if ProjectState._rebuildIcons then
        ...
        if tabsChanged or ProjectState._iconAccentKey ~= ikey then
            ...
            if tabsChanged or not ProjectState._iconGenT or (not ProjectState.rainbow and (nowc - ProjectState._iconGenT) >= 0.13) then
...
local function runStepSafe()
    ...
    if not ok then
        print("[Celestial UI] render error: " .. tostring(err))
...
local function frameDelay()
    ...
    local overUi = false
    local winRect = ProjectState._winRect
    if ProjectState.open and winRect then
        overUi = over(winRect.x, winRect.y, winRect.w, winRect.h)
    elseif ProjectState.minimized and ProjectState.minPos then
        overUi = over(ProjectState.minPos.x, ProjectState.minPos.y, 42, 42)
    end
    if (moved and overUi)
...
function ui:CreateWindow(config)
    ...
    if config.neverloseMode ~= nil then ProjectState.neverloseMode = config.neverloseMode == true end
    ...
    if config.lockChrome == true then
        ProjectState.lockedChrome = {
            bg = Theme.bg, sidebar = Theme.sidebar, text = Theme.text, sub = Theme.sub,
            surface = Theme.surface, surface2 = Theme.surface2, surface3 = Theme.surface3,
            border = Theme.border, trackOff = Theme.trackOff, sliderTrack = Theme.sliderTrack,
            ...
            neverloseMode = ProjectState.neverloseMode,
            searchStyle = ProjectState.searchStyle, layout = ProjectState.tabLayout,
            menuKey = menuKey,
            fontName = ProjectState.uiFontName, font = ProjectState.uiFont,
            w = ProjectState.w, h = ProjectState.h,
            card = AL.card, cardStrk = AL.cardStrk, hairline = AL.hairline,
        }
    end
    ...
    task.spawn(function()
        ...
        local uid, uname
        for _ = 1, 30 do
            if not ProjectState.alive then return end
